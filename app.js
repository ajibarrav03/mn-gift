...
function backupPayload(){
  return {
    app:'M&N Gift', version:'V.MN.0.0.008', phase:2, currency:'COP', exportedAt:new Date().toISOString(),
    data:{
...
// ============================================================
// BLOQUE FINANCIERO AVANZADO — desarrollo sobre V.MN.0.0.008
// ============================================================
...
function renderAccountsReceivable(){
  ...
  rows.forEach(r=>{const paid=receivableCollected(r),balance=receivableBalance(r),st=receivableDueState(r);collected+=paid;if(balance>.009){pending+=balance;if(dayDifference(r.dueDate)<0)overdue+=balance}const hasCollections=receivableCollections(r).length>0,actions=balance>.009?`<button class="small-btn" type="button" data-collect-receivable="${r.id}">Cobrar</button><button class="small-btn" type="button" data-edit-receivable="${r.id}">Editar</button>${hasCollections?`<button class="small-btn" type="button" data-revert-receivable-collection="${r.id}">Revertir último cobro</button>`:`<button class="small-btn danger" type="button" data-delete-receivable="${r.id}">Eliminar</button>`}`:`${hasCollections?`<button class="small-btn" type="button" data-revert-receivable-collection="${r.id}">Revertir último cobro</button>`:''}`;const tr=document.createElement('tr');tr.innerHTML=`<td>${niceDate(r.dueDate)}</td><td><strong>${escapeHtml(r.debtor)}</strong></td><td>${escapeHtml(r.concept)}</td><td>${COP.format(r.amount)}</td><td class="profit">${COP.format(paid)}</td><td class="${balance>.009?'expense-amount':'profit'}">${COP.format(balance)}</td><td><span class="payment-method-pill">${escapeHtml(r.plannedMethod||'Otro')}</span></td><td><span class="payable-status ${st.cls}">${escapeHtml(st.label)}</span></td><td><div class="row-actions">${actions||'—'}</div></td>`;tb.appendChild(tr)});
...
// ============================================================
// BLOQUE PRODUCCIÓN Y PLANIFICACIÓN — desarrollo V.MN.0.0.008
// ============================================================
