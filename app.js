...
const state = {
  ...
  accountsPayable: JSON.parse(localStorage.getItem('mngifts_accounts_payable') || '[]'),
  accountsReceivable: JSON.parse(localStorage.getItem('mngifts_accounts_receivable') || '[]'),
  cashAdjustments: JSON.parse(localStorage.getItem('mngifts_cash_adjustments') || '[]'),
...
let draftRecipe = [];
let editing = { material:null, purchase:null, product:null, sale:null, expense:null, customer:null, order:null, supplier:null, payable:null, receivable:null, cashAdjustment:null };
...
function save(){
  ...
  localStorage.setItem('mngifts_accounts_payable',JSON.stringify(state.accountsPayable));
  localStorage.setItem('mngifts_accounts_receivable',JSON.stringify(state.accountsReceivable));
  localStorage.setItem('mngifts_cash_adjustments',JSON.stringify(state.cashAdjustments));
...
function backupPayload(){
  return {
    app:'M&N Gift', version:'V.MN.0.0.009', phase:2, currency:'COP', exportedAt:new Date().toISOString(),
    data:{
      materials:state.materials, products:state.products, sales:state.sales, purchases:state.purchases,
      expenses:state.expenses, customers:state.customers, orders:state.orders, payments:state.payments, cashClosures:state.cashClosures, accountsPayable:state.accountsPayable, accountsReceivable:state.accountsReceivable, cashAdjustments:state.cashAdjustments, suppliers:state.suppliers, inventoryMovements:state.inventoryMovements, productionJobs:state.productionJobs, productionReservations:state.productionReservations, customCategories:state.customCategories
    }
  };
}
...
function validateBackup(payload){
  ...
  const keys=['materials','products','sales','purchases','expenses','customers','orders','customCategories'];if(!Array.isArray(d.payments))d.payments=[];if(!Array.isArray(d.cashClosures))d.cashClosures=[];if(!Array.isArray(d.accountsPayable))d.accountsPayable=[];if(!Array.isArray(d.accountsReceivable))d.accountsReceivable=[];if(!Array.isArray(d.cashAdjustments))d.cashAdjustments=[];if(!Array.isArray(d.suppliers))d.suppliers=[];if(!Array.isArray(d.inventoryMovements))d.inventoryMovements=[];if(!Array.isArray(d.productionJobs))d.productionJobs=[];if(!Array.isArray(d.productionReservations))d.productionReservations=[];
...
function clearAllBusinessData(){
  ...
  localStorage.setItem('mngifts_emergency_backup',JSON.stringify(backupPayload()));
  ['materials','products','sales','purchases','expenses','customers','orders','payments','cashClosures','accountsPayable','accountsReceivable','cashAdjustments','suppliers','inventoryMovements','productionJobs','productionReservations','customCategories'].forEach(k=>state[k]=[]);save();renderAll();alert('Los datos locales fueron borrados. Se guardó una copia de emergencia temporal en este navegador.');
}
...
function renderAll(){ migrateFinanceData();const invMigrated=migrateInventoryData();if(invMigrated)save();renderDailyQuote();refreshCategoryOptions();renderMaterials();renderInventory();renderPurchases();renderProducts();renderCustomers();renderOrders();renderProduction();renderSales();renderExpenses();renderDashboard();renderReports();renderAccountsPayable();renderAccountsReceivable();renderCashAdjustments();renderCashMovements();renderCashClosure();refreshPurchaseMaterials();refreshSupplierSelects();refreshOrderCustomers();refreshOrderProducts();renderDataSummary();renderBusinessAlerts();enhanceMobileTables();renderMobileNav(); }
...
document.getElementById('expenseForm').addEventListener('submit',e=>{e.preventDefault();if(!requireBusinessAdmin())return;const d=Object.fromEntries(new FormData(e.currentTarget)),data={category:d.category,description:d.description.trim(),amount:num(d.amount),date:d.date,paymentMethod:d.paymentMethod||'Otro'};if(data.category==='Pago a empleados'){const budget=employeePayrollBudget(editing.expense);if(data.amount>budget.available+0.01){alert(`El pago supera el monto disponible para empleados.\n\nDisponible actualmente: ${COP.format(budget.available)}\nTope de nómina: ${COP.format(budget.limit)} (90% de la ganancia neta antes de pagos a empleados).`);return}}if(editing.expense)Object.assign(state.expenses.find(x=>x.id===editing.expense),data);else state.expenses.push({id:crypto.randomUUID(),...data});save();renderAll();closeModal('expenseModal');resetExpenseModal()});
...
${summary}`;if(confirm(msg)){restore(plan.combined);plan.lines.forEach(u=>recordInventoryMovement({materialId:u.materialId,kind:'sale_reversal',quantity:u.quantity,date:todayISO(),note:`Reversión de venta: ${u.productName} × ${formatQty(u.productQuantity)}`,sourceType:'sale_reversal',sourceId:s.id,unitCost:u.unitCost}));state.sales=state.sales.filter(x=>x.id!==s.id);if(!s.orderId)state.payments=state.payments.filter(p=>p.saleId!==s.id);if(s.orderId){const o=getOrder(s.orderId);if(o){o.saleId=null;if(o.status==='Entregado')o.status='Listo';delete o.convertedAt}}save();renderAll()}return;}
  ...
  if(t.dataset.editExpense){if(!requireBusinessAdmin())return;const linked=state.accountsPayable.find(p=>p.expenseId===t.dataset.editExpense&&p.status==='Pagada');if(linked){alert('Este gasto proviene de una cuenta por pagar. Para modificarlo, reabre primero la cuenta desde Caja → Cuentas por pagar y vuelve a registrarla.');return}return openExpenseEdit(t.dataset.editExpense);}
  if(t.dataset.deleteExpense){if(!requireBusinessAdmin())return;const linked=state.accountsPayable.find(p=>p.expenseId===t.dataset.deleteExpense&&p.status==='Pagada');if(linked){alert('Este gasto proviene de una cuenta por pagar. Para conservar la trazabilidad, reabre primero la cuenta desde Caja → Cuentas por pagar.');return}if(confirm('¿Eliminar este gasto?')){state.expenses=state.expenses.filter(x=>x.id!==t.dataset.deleteExpense);save();renderAll()}return;}
});



// ============================================================
// BLOQUE FINANCIERO AVANZADO — desarrollo sobre V.MN.0.0.009
// ============================================================
...
// ============================================================
// CAJA — NÓMINA DISPONIBLE + CUENTAS POR PAGAR
// Desarrollo sobre V.MN.0.0.009
// ============================================================
function employeePayrollBudget(excludeExpenseId=null){
  const salesRevenue=state.sales.reduce((sum,s)=>sum+num(s.total),0);
  const salesCost=state.sales.reduce((sum,s)=>sum+num(s.cost),0);
  const considered=state.expenses.filter(x=>x.id!==excludeExpenseId);
  const employeePaid=considered.filter(x=>x.category==='Pago a empleados').reduce((sum,x)=>sum+num(x.amount),0);
  const otherExpenses=considered.filter(x=>x.category!=='Pago a empleados').reduce((sum,x)=>sum+num(x.amount),0);
  const baseNet=Math.max(0,salesRevenue-salesCost-otherExpenses);
  const limit=Math.max(0,baseNet*.90);
  const available=Math.max(0,limit-employeePaid);
  const currentNet=salesRevenue-salesCost-otherExpenses-employeePaid;
  return {salesRevenue,salesCost,otherExpenses,baseNet,limit,employeePaid,available,currentNet};
}
function showEmployeePayrollAvailability(){
  const b=employeePayrollBudget(),box=document.getElementById('employeePayAvailabilityResult');if(!box)return;
  box.hidden=false;
  box.innerHTML=`<div class="employee-pay-available"><span>Disponible para pagar a empleados</span><strong>${COP.format(b.available)}</strong></div><div class="employee-pay-breakdown"><div><span>Ganancia neta antes de nómina</span><strong>${COP.format(b.baseNet)}</strong></div><div><span>Tope permitido (90%)</span><strong>${COP.format(b.limit)}</strong></div><div><span>Ya pagado a empleados</span><strong>${COP.format(b.employeePaid)}</strong></div><div><span>Ganancia neta actual</span><strong>${COP.format(b.currentNet)}</strong></div></div><small>El sistema bloquea nuevos pagos a empleados que hagan superar el 90% permitido.</small>`;
}
function hideCashInlinePanels(){
  document.querySelectorAll('.cash-inline-panel').forEach(x=>x.hidden=true);
}
function showCashInlinePanel(id){
  hideCashInlinePanels();
  const panel=document.getElementById(id);if(!panel)return;panel.hidden=false;
  requestAnimationFrame(()=>panel.scrollIntoView({behavior:'smooth',block:'nearest'}));
}
function focusCashMovements(){
  const panel=document.querySelector('.cash-movements-panel');if(!panel)return;
  panel.classList.add('cash-movement-updated');
  panel.scrollIntoView({behavior:'smooth',block:'start'});
  setTimeout(()=>panel.classList.remove('cash-movement-updated'),1400);
}
document.addEventListener('click',e=>{if(e.target.closest('[data-close-cash-inline]')){hideCashInlinePanels();editing.payable=null;editing.receivable=null;editing.cashAdjustment=null;}},true);
document.getElementById('calculateEmployeePayAvailable')?.addEventListener('click',()=>{showCashInlinePanel('employeePayInlinePanel');showEmployeePayrollAvailability()});

function payableStatus(p){return p.status==='Pagada'?'Pagada':'Pendiente'}
function resetPayableModal(){editing.payable=null;const f=document.getElementById('payableForm');if(!f)return;f.reset();f.elements.dueDate.value=todayISO();f.elements.plannedMethod.value='Efectivo';document.getElementById('payableModalTitle').textContent='Agregar cuenta por pagar';document.getElementById('payableSubmitBtn').textContent='Guardar cuenta por pagar'}
function openPayableModal(id=null){
  resetPayableModal();
  if(id){const p=state.accountsPayable.find(x=>x.id===id);if(!p||p.status==='Pagada')return;editing.payable=id;const f=document.getElementById('payableForm');f.elements.creditor.value=p.creditor||'';f.elements.concept.value=p.concept||'';f.elements.amount.value=num(p.amount);f.elements.dueDate.value=p.dueDate||todayISO();f.elements.plannedMethod.value=p.plannedMethod||'Otro';f.elements.notes.value=p.notes||'';document.getElementById('payableModalTitle').textContent='Editar cuenta por pagar';document.getElementById('payableSubmitBtn').textContent='Guardar cambios'}
  showCashInlinePanel('payableInlinePanel');
}
function payableDueState(p){if(p.status==='Pagada')return {label:'Pagada',cls:'paid'};const days=dayDifference(p.dueDate);if(days===null)return {label:'Pendiente',cls:'pending'};if(days<0)return {label:`Vencida ${Math.abs(days)}d`,cls:'overdue'};if(days===0)return {label:'Vence hoy',cls:'due'};return {label:`Pendiente · ${days}d`,cls:'pending'}}
function renderAccountsPayable(){
  const tb=document.getElementById('accountsPayableTable'),empty=document.getElementById('accountsPayableEmpty');if(!tb||!empty)return;
  const rows=[...state.accountsPayable].sort((a,b)=>{const ap=a.status==='Pagada'?1:0,bp=b.status==='Pagada'?1:0;return ap-bp||String(a.dueDate||'').localeCompare(String(b.dueDate||''))});
  tb.innerHTML='';empty.style.display=rows.length?'none':'block';
  let pending=0,overdue=0;
  rows.forEach(p=>{if(p.status!=='Pagada'){pending+=num(p.amount);if(dayDifference(p.dueDate)<0)overdue+=num(p.amount)}const st=payableDueState(p),tr=document.createElement('tr'),actions=p.status==='Pagada'?`<button class="small-btn reopen" type="button" data-reopen-payable="${p.id}">Reabrir</button>`:`<button class="small-btn pay" type="button" data-pay-payable="${p.id}">Pagar</button><button class="small-btn edit" type="button" data-edit-payable="${p.id}">Editar</button><button class="small-btn danger" type="button" data-delete-payable="${p.id}">Eliminar</button>`;tr.innerHTML=`<td>${niceDate(p.dueDate)}</td><td><strong>${escapeHtml(p.creditor)}</strong></td><td>${escapeHtml(p.concept)}</td><td>${COP.format(p.amount)}</td><td><span class="payment-method-pill">${escapeHtml(p.status==='Pagada'?(p.paidMethod||p.plannedMethod||'Otro'):(p.plannedMethod||'Otro'))}</span></td><td><span class="payable-status ${st.cls}">${escapeHtml(st.label)}</span>${p.status==='Pagada'&&p.paidDate?`<br><small>Pagada ${niceDate(p.paidDate)}</small>`:''}</td><td><div class="row-actions">${actions}</div></td>`;tb.appendChild(tr)});
  const p=document.getElementById('accountsPayablePending'),o=document.getElementById('accountsPayableOverdue');if(p)p.textContent=COP.format(pending);if(o)o.textContent=COP.format(overdue);const payroll=document.getElementById('employeePayAvailabilityResult');if(payroll&&!payroll.hidden)showEmployeePayrollAvailability();
}
document.getElementById('openPayableModal')?.addEventListener('click',()=>openPayableModal());
document.getElementById('payableForm')?.addEventListener('submit',e=>{e.preventDefault();if(!requireBusinessAdmin())return;const d=Object.fromEntries(new FormData(e.currentTarget)),data={creditor:d.creditor.trim(),concept:d.concept.trim(),amount:num(d.amount),dueDate:d.dueDate,plannedMethod:d.plannedMethod||'Otro',notes:d.notes?.trim()||''};if(data.amount<=0){alert('El monto debe ser mayor que cero.');return}if(editing.payable){Object.assign(state.accountsPayable.find(x=>x.id===editing.payable),data)}else state.accountsPayable.push({id:crypto.randomUUID(),...data,status:'Pendiente',createdAt:new Date().toISOString(),paidDate:null,paidMethod:null,expenseId:null});save();renderAll();hideCashInlinePanels();resetPayableModal()});
function openPayablePayment(id){const p=state.accountsPayable.find(x=>x.id===id);if(!p||p.status==='Pagada')return;const f=document.getElementById('payablePaymentForm');f.reset();f.elements.payableId.value=p.id;f.elements.date.value=todayISO();f.elements.method.value=PAYMENT_METHODS.includes(p.plannedMethod)?p.plannedMethod:'Otro';document.getElementById('payablePaymentTarget').innerHTML=`<strong>${escapeHtml(p.creditor)}</strong><br>${escapeHtml(p.concept)} · <strong>${COP.format(p.amount)}</strong><br>Vencimiento: ${niceDate(p.dueDate)}`;openModal('payablePaymentModal')}
document.getElementById('payablePaymentForm')?.addEventListener('submit',e=>{e.preventDefault();if(!requireBusinessAdmin())return;const d=Object.fromEntries(new FormData(e.currentTarget)),p=state.accountsPayable.find(x=>x.id===d.payableId);if(!p||p.status==='Pagada')return;const expenseId=crypto.randomUUID(),paidDate=d.date||todayISO(),method=d.method||p.plannedMethod||'Otro';state.expenses.push({id:expenseId,category:'Cuenta por pagar',description:`${p.creditor} · ${p.concept}`,amount:num(p.amount),date:paidDate,paymentMethod:method});Object.assign(p,{status:'Pagada',paidDate,paidMethod:method,expenseId});save();renderAll();closeModal('payablePaymentModal');focusCashMovements();alert(`Cuenta pagada y registrada como egreso por ${COP.format(p.amount)}.`)});
document.addEventListener('click',e=>{const t=e.target.closest('[data-edit-payable],[data-delete-payable],[data-pay-payable],[data-reopen-payable]');if(!t)return;if(t.dataset.editPayable){openPayableModal(t.dataset.editPayable);return}if(t.dataset.deletePayable){const p=state.accountsPayable.find(x=>x.id===t.dataset.deletePayable);if(!p||p.status==='Pagada')return;if(confirm(`¿Eliminar la cuenta por pagar “${p.concept}”?`)){state.accountsPayable=state.accountsPayable.filter(x=>x.id!==p.id);save();renderAll()}return}if(t.dataset.payPayable){openPayablePayment(t.dataset.payPayable);return}if(t.dataset.reopenPayable){const p=state.accountsPayable.find(x=>x.id===t.dataset.reopenPayable);if(!p||p.status!=='Pagada')return;if(!confirm(`¿Reabrir la cuenta “${p.concept}”?\n\nSe eliminará el egreso vinculado y volverá a quedar pendiente.`))return;if(p.expenseId)state.expenses=state.expenses.filter(x=>x.id!==p.expenseId);Object.assign(p,{status:'Pendiente',paidDate:null,paidMethod:null,expenseId:null});save();renderAll();return}},true);


// ============================================================
// CAJA — CUENTAS POR COBRAR + AJUSTES DE INGRESOS / EGRESOS
// Desarrollo sobre V.MN.0.0.009
// ============================================================
function receivableCollections(r){return Array.isArray(r?.collections)?r.collections:[]}
function receivableCollected(r){return receivableCollections(r).reduce((sum,x)=>sum+num(x.amount),0)}
function receivableBalance(r){return Math.max(0,num(r?.amount)-receivableCollected(r))}
function receivableStatus(r){const paid=receivableCollected(r),balance=receivableBalance(r);if(balance<=0.009)return 'Cobrada';if(paid>0)return 'Parcial';return 'Pendiente'}
function receivableDueState(r){const status=receivableStatus(r);if(status==='Cobrada')return {label:'Cobrada',cls:'paid'};const days=dayDifference(r.dueDate),prefix=status==='Parcial'?'Parcial':'Pendiente';if(days===null)return {label:prefix,cls:'pending'};if(days<0)return {label:`${prefix} · vencida ${Math.abs(days)}d`,cls:'overdue'};if(days===0)return {label:`${prefix} · vence hoy`,cls:'due'};return {label:`${prefix} · ${days}d`,cls:'pending'}}
function resetReceivableModal(){editing.receivable=null;const f=document.getElementById('receivableForm');if(!f)return;f.reset();f.elements.dueDate.value=todayISO();f.elements.plannedMethod.value='Efectivo';document.getElementById('receivableModalTitle').textContent='Agregar cuenta por cobrar';document.getElementById('receivableSubmitBtn').textContent='Guardar cuenta por cobrar'}
function openReceivableModal(id=null){
  resetReceivableModal();
  if(id){const r=state.accountsReceivable.find(x=>x.id===id);if(!r)return;editing.receivable=id;const f=document.getElementById('receivableForm');f.elements.debtor.value=r.debtor||'';f.elements.concept.value=r.concept||'';f.elements.amount.value=num(r.amount);f.elements.dueDate.value=r.dueDate||todayISO();f.elements.plannedMethod.value=r.plannedMethod||'Otro';f.elements.notes.value=r.notes||'';document.getElementById('receivableModalTitle').textContent='Editar cuenta por cobrar';document.getElementById('receivableSubmitBtn').textContent='Guardar cambios'}
  showCashInlinePanel('receivableInlinePanel');
}
function renderAccountsReceivable(){
  const tb=document.getElementById('accountsReceivableTable'),empty=document.getElementById('accountsReceivableEmpty');if(!tb||!empty)return;
  const rows=[...state.accountsReceivable].sort((a,b)=>{const ap=receivableBalance(a)<=.009?1:0,bp=receivableBalance(b)<=.009?1:0;return ap-bp||String(a.dueDate||'').localeCompare(String(b.dueDate||''))});
  tb.innerHTML='';empty.style.display=rows.length?'none':'block';let pending=0,overdue=0,collected=0;
  rows.forEach(r=>{const paid=receivableCollected(r),balance=receivableBalance(r),st=receivableDueState(r);collected+=paid;if(balance>.009){pending+=balance;if(dayDifference(r.dueDate)<0)overdue+=balance}const hasCollections=receivableCollections(r).length>0,actions=balance>.009?`<button class="small-btn collect" type="button" data-collect-receivable="${r.id}">Cobrar</button><button class="small-btn edit" type="button" data-edit-receivable="${r.id}">Editar</button>${hasCollections?`<button class="small-btn revert" type="button" data-revert-receivable-collection="${r.id}">Revertir último cobro</button>`:`<button class="small-btn danger" type="button" data-delete-receivable="${r.id}">Eliminar</button>`}`:`${hasCollections?`<button class="small-btn revert" type="button" data-revert-receivable-collection="${r.id}">Revertir último cobro</button>`:''}`;const tr=document.createElement('tr');tr.innerHTML=`<td>${niceDate(r.dueDate)}</td><td><strong>${escapeHtml(r.debtor)}</strong></td><td>${escapeHtml(r.concept)}</td><td>${COP.format(r.amount)}</td><td class="profit">${COP.format(paid)}</td><td class="${balance>.009?'expense-amount':'profit'}">${COP.format(balance)}</td><td><span class="payment-method-pill">${escapeHtml(r.plannedMethod||'Otro')}</span></td><td><span class="payable-status ${st.cls}">${escapeHtml(st.label)}</span></td><td><div class="row-actions">${actions||'—'}</div></td>`;tb.appendChild(tr)});
  const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=COP.format(v)};set('accountsReceivablePending',pending);set('accountsReceivableOverdue',overdue);set('accountsReceivableCollected',collected);
}
document.getElementById('openReceivableModal')?.addEventListener('click',()=>openReceivableModal());
document.getElementById('receivableForm')?.addEventListener('submit',e=>{e.preventDefault();if(!requireBusinessAdmin())return;const d=Object.fromEntries(new FormData(e.currentTarget)),data={debtor:d.debtor.trim(),concept:d.concept.trim(),amount:num(d.amount),dueDate:d.dueDate,plannedMethod:d.plannedMethod||'Otro',notes:d.notes?.trim()||''};if(data.amount<=0){alert('El monto debe ser mayor que cero.');return}if(editing.receivable){const r=state.accountsReceivable.find(x=>x.id===editing.receivable);if(!r)return;const paid=receivableCollected(r);if(data.amount+0.01<paid){alert(`El total no puede quedar por debajo de lo ya cobrado (${COP.format(paid)}).`);return}Object.assign(r,data,{status:receivableStatus({...r,...data})})}else state.accountsReceivable.push({id:crypto.randomUUID(),...data,status:'Pendiente',collections:[],createdAt:new Date().toISOString()});save();renderAll();hideCashInlinePanels();resetReceivableModal()});
function openReceivableCollection(id){const r=state.accountsReceivable.find(x=>x.id===id);if(!r)return;const balance=receivableBalance(r);if(balance<=.009){alert('Esta cuenta ya está totalmente cobrada.');return}const f=document.getElementById('receivableCollectionForm');f.reset();f.elements.receivableId.value=r.id;f.elements.date.value=todayISO();f.elements.amount.value=Math.round(balance);f.elements.amount.max=Math.max(1,Math.ceil(balance));f.elements.method.value=PAYMENT_METHODS.includes(r.plannedMethod)?r.plannedMethod:'Otro';document.getElementById('receivableCollectionTarget').innerHTML=`<strong>${escapeHtml(r.debtor)}</strong><br>${escapeHtml(r.concept)} · Saldo pendiente: <strong>${COP.format(balance)}</strong>`;openModal('receivableCollectionModal')}
document.getElementById('receivableCollectionForm')?.addEventListener('submit',e=>{e.preventDefault();if(!requireBusinessAdmin())return;const d=Object.fromEntries(new FormData(e.currentTarget)),r=state.accountsReceivable.find(x=>x.id===d.receivableId);if(!r)return;const amount=num(d.amount),balance=receivableBalance(r);if(amount<=0||amount>balance+0.01){alert(`El cobro debe ser mayor que cero y no superar ${COP.format(balance)}.`);return}if(!Array.isArray(r.collections))r.collections=[];r.collections.push({id:crypto.randomUUID(),date:d.date||todayISO(),amount,method:d.method||'Otro',note:d.note?.trim()||''});r.status=receivableStatus(r);save();renderAll();closeModal('receivableCollectionModal');focusCashMovements();alert(`Cobro registrado por ${COP.format(amount)}. Saldo pendiente: ${COP.format(receivableBalance(r))}.`)});
document.addEventListener('click',e=>{const t=e.target.closest('[data-edit-receivable],[data-delete-receivable],[data-collect-receivable],[data-revert-receivable-collection]');if(!t)return;if(t.dataset.editReceivable){openReceivableModal(t.dataset.editReceivable);return}if(t.dataset.deleteReceivable){const r=state.accountsReceivable.find(x=>x.id===t.dataset.deleteReceivable);if(!r)return;if(receivableCollections(r).length){alert('Primero revierte los cobros registrados para poder eliminar esta cuenta.');return}if(confirm(`¿Eliminar la cuenta por cobrar “${r.concept}”?`)){state.accountsReceivable=state.accountsReceivable.filter(x=>x.id!==r.id);save();renderAll()}return}if(t.dataset.collectReceivable){openReceivableCollection(t.dataset.collectReceivable);return}if(t.dataset.revertReceivableCollection){const r=state.accountsReceivable.find(x=>x.id===t.dataset.revertReceivableCollection);if(!r||!receivableCollections(r).length)return;const last=r.collections[r.collections.length-1];if(!confirm(`¿Revertir el último cobro de ${COP.format(last.amount)} registrado el ${niceDate(last.date)}?`))return;r.collections.pop();r.status=receivableStatus(r);save();renderAll();return}},true);

function resetCashAdjustmentModal(){editing.cashAdjustment=null;const f=document.getElementById('cashAdjustmentForm');if(!f)return;f.reset();f.elements.date.value=todayISO();f.elements.type.value='Ingreso';f.elements.method.value='Efectivo';document.getElementById('cashAdjustmentModalTitle').textContent='Registrar ajuste de caja';document.getElementById('cashAdjustmentSubmitBtn').textContent='Guardar ajuste'}
function openCashAdjustmentModal(id=null){resetCashAdjustmentModal();if(id){const x=state.cashAdjustments.find(a=>a.id===id);if(!x)return;editing.cashAdjustment=id;const f=document.getElementById('cashAdjustmentForm');f.elements.type.value=x.type||'Ingreso';f.elements.date.value=x.date||todayISO();f.elements.amount.value=num(x.amount);f.elements.method.value=x.method||'Otro';f.elements.concept.value=x.concept||'';f.elements.reason.value=x.reason||'';f.elements.notes.value=x.notes||'';document.getElementById('cashAdjustmentModalTitle').textContent='Editar ajuste de caja';document.getElementById('cashAdjustmentSubmitBtn').textContent='Guardar cambios'}showCashInlinePanel('cashAdjustmentInlinePanel')}
function renderCashAdjustments(){const tb=document.getElementById('cashAdjustmentsTable'),empty=document.getElementById('cashAdjustmentsEmpty');if(!tb||!empty)return;const rows=[...state.cashAdjustments].sort((a,b)=>(b.date||'').localeCompare(a.date||'')||(b.createdAt||'').localeCompare(a.createdAt||''));tb.innerHTML='';empty.style.display=rows.length?'none':'block';rows.forEach(x=>{const tr=document.createElement('tr');tr.innerHTML=`<td>${niceDate(x.date)}</td><td><strong class="${x.type==='Ingreso'?'profit':'expense-amount'}">${escapeHtml(x.type)}</strong></td><td><strong>${escapeHtml(x.concept)}</strong><br><small>${escapeHtml(x.reason)}</small></td><td><span class="payment-method-pill">${escapeHtml(x.method||'Otro')}</span></td><td class="${x.type==='Ingreso'?'profit':'expense-amount'}">${COP.format(x.amount)}</td><td><div class="row-actions"><button class="small-btn edit" type="button" data-edit-cash-adjustment="${x.id}">Editar</button><button class="small-btn danger" type="button" data-delete-cash-adjustment="${x.id}">Eliminar</button></div></td>`;tb.appendChild(tr)})}
document.getElementById('openCashAdjustmentModal')?.addEventListener('click',()=>openCashAdjustmentModal());
document.getElementById('cashAdjustmentForm')?.addEventListener('submit',e=>{e.preventDefault();if(!requireBusinessAdmin())return;const d=Object.fromEntries(new FormData(e.currentTarget)),data={type:d.type==='Egreso'?'Egreso':'Ingreso',date:d.date||todayISO(),amount:num(d.amount),method:d.method||'Otro',concept:d.concept.trim(),reason:d.reason.trim(),notes:d.notes?.trim()||''};if(data.amount<=0){alert('El monto debe ser mayor que cero.');return}if(editing.cashAdjustment){Object.assign(state.cashAdjustments.find(x=>x.id===editing.cashAdjustment),data)}else state.cashAdjustments.push({id:crypto.randomUUID(),...data,createdAt:new Date().toISOString()});save();renderAll();hideCashInlinePanels();resetCashAdjustmentModal();focusCashMovements()});
document.addEventListener('click',e=>{const t=e.target.closest('[data-edit-cash-adjustment],[data-delete-cash-adjustment]');if(!t)return;if(t.dataset.editCashAdjustment){openCashAdjustmentModal(t.dataset.editCashAdjustment);return}if(t.dataset.deleteCashAdjustment){const x=state.cashAdjustments.find(a=>a.id===t.dataset.deleteCashAdjustment);if(x&&confirm(`¿Eliminar el ajuste “${x.concept}” por ${COP.format(x.amount)}?`)){state.cashAdjustments=state.cashAdjustments.filter(a=>a.id!==x.id);save();renderAll()}return}},true);
...
function allCashMovements(){
  ...
  state.accountsReceivable.forEach(r=>receivableCollections(r).forEach(col=>{const detail=`Cuenta por cobrar · ${r.debtor||'Deudor'} · ${r.concept||'Cobro'}${col.note?` · ${col.note}`:''}`;rows.push({id:`receivable-${r.id}-${col.id}`,date:col.date||todayISO(),type:'Ingreso',category:'Cuenta por cobrar',detail,method:col.method||r.plannedMethod||'Otro',income:num(col.amount),out:0,sequence:sequence++})}));
  state.cashAdjustments.forEach(x=>{const income=x.type==='Ingreso'?num(x.amount):0,out=x.type==='Egreso'?num(x.amount):0,detail=`${x.concept||'Ajuste de caja'}${x.reason?` · Motivo: ${x.reason}`:''}${x.notes?` · ${x.notes}`:''}`;rows.push({id:`adjustment-${x.id}`,date:x.date||todayISO(),type:x.type==='Egreso'?'Egreso':'Ingreso',category:'Ajuste de caja',detail,method:x.method||'Otro',income,out,sequence:sequence++})});
...
function cashActivityForDate(date){const rows=allCashMovements().filter(x=>x.date===date&&x.method==='Efectivo');return {income:rows.reduce((sum,x)=>sum+num(x.income),0),out:rows.reduce((sum,x)=>sum+num(x.out),0)}}
...
// ============================================================
// BLOQUE PRODUCCIÓN Y PLANIFICACIÓN — desarrollo V.MN.0.0.009
// ============================================================
